<?php
class buddha_Validator_Validator extends Buddha_Base_Component
{
    /**
     ...
     */
    public function __construct(Buddha_Validator_TranslatorInterface $translator, array $data, array $rules, array $messages = array(), array $customAttributes = array())
    {
    ...
    public function after($callback)
    {
        $this->after[] = function () use ($callback) {
            return call_user_func($callback, array(), 'validate');
        };
    ...
    {
        $data = Buddha_Validator_Helpers::array_get($this->data, $attribute);
    ...
    {
        $this->messages = new Buddha_Validator_MessageBag();
    ...
    {
        if (!is_null($value = Buddha_Validator_Helpers::array_get($this->data, $attribute))) {
            return $value;
        } elseif (!is_null($value = Buddha_Validator_Helpers::array_get($this->files, $attribute))) {
            return $value;
        }
    }
    ...
    protected function passesOptionalCheck($attribute)
    {
        if ($this->hasRule($attribute, array('Sometimes'))) {
            return array_key_exists($attribute, Buddha_Validator_Helpers::array_dot($this->data))
    ...
    protected function addFailure($attribute, $rule, $parameters)
    {
        ...
        $this->failedRules[$attribute][Buddha_Validator_Helpers::snake_case($rule)] = $parameters;
    }
    ...
    protected function validateRequiredIf($attribute, $value, $parameters)
    {
        ...
        $data = Buddha_Validator_Helpers::array_get($this->data, $parameters[0]);
    ...
    protected function getPresentCount($attributes)
    {
        ...
        foreach ($attributes as $key) {
            if (Buddha_Validator_Helpers::array_get($this->data, $key) || Buddha_Validator_Helpers::array_get($this->files, $key)) {
                ++$count;
            }
        }
    ...
    protected function validateSame($attribute, $value, $parameters)
    {
        ...
        $other = Buddha_Validator_Helpers::array_get($this->data, $parameters[0]);
    ...
    protected function getSize($attribute, $value)
    {
        ...
        if (is_numeric($value) && $hasNumeric) {
            return Buddha_Validator_Helpers::array_get($this->data, $attribute);
    ...
    {
        $lowerRule = Buddha_Validator_Helpers::snake_case($rule);
    ...
    protected function doReplacements($message, $attribute, $rule, $parameters)
    {
        ...
        if (isset($this->replacers[Buddha_Validator_Helpers::snake_case($rule)])) {
            $message = $this->callReplacer($message, $attribute, Buddha_Validator_Helpers::snake_case($rule), $parameters);
    ...
    protected function getAttribute($attribute)
    {
        ...
        // used as default versions of the attribute's displayable names.
            ...
            return str_replace('_', ' ', Buddha_Validator_Helpers::snake_case($attribute));
        }
    }
    ...
    {
        $parameters[1] = $this->getDisplayableValue($parameters[0], Buddha_Validator_Helpers::array_get($this->data, $parameters[0]));
    ...
    {
        return array(Buddha_Validator_Helpers::studly_case(trim(Buddha_Validator_Helpers::array_get($rules, 0))), array_slice($rules, 1));
    }
    ...
    protected function parseStringRule($rules)
    {
        ...
        return array(Buddha_Validator_Helpers::studly_case(trim($rules)), $parameters);
    }
    ...
    public function addImplicitExtensions(array $extensions)
    {
        ...
        foreach ($extensions as $rule => $extension) {
            $this->implicitRules[] = Buddha_Validator_Helpers::studly_case($rule);
        }
    }
    ...
    {
        $this->extensions[Buddha_Validator_Helpers::snake_case($rule)] = $extension;
    }
    ...
    {
        $this->replacers[Buddha_Validator_Helpers::snake_case($rule)] = $replacer;
    }
     ...
     *
     * @param Buddha_Validator_TranslatorInterface
     */
    public function setTranslator(Buddha_Validator_TranslatorInterface $translator)
    {
        $this->translator = $translator;
    }
    ...
    protected function callClassBasedReplacer($callback, $message, $attribute, $rule, $parameters)
    {
        ...
        return call_user_func_array(array($class, $method), array_slice(func_get_args(), 1));
    }
    ...
    {
        $rule = Buddha_Validator_Helpers::snake_case(substr($method, 8));
